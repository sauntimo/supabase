...
Under `OAuth & Permissions > Scopes > User Token Scopes`

Sign in with Slack requires three User Scope Tokens as explained in [the documentation](https://api.slack.com/authentication/sign-in-with-slack#implementation). These scopes are requested by default by the [GoTrue slack provider](https://github.com/supabase/gotrue/blob/master/api/provider/slack.go).

- Click `Add an OAuth Scope` and add `openid`, `profile` and `email`
